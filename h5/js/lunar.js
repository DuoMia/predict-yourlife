(function () {
  ...
  function calculate(obj) {
    ...
    var theYear = (obj.year) % MINYEAR;
    var ruleArr = time[theYear];
    var nowDay = parseInt((new Date(obj.year + '/' + obj.month + '/' + obj.day) - new Date(obj.year + '/1/1')) / (24 * 60 * 60 * 1000)) + 1;
    var nowYear = ruleArr[0] > 0 ? obj.year-- : obj.year;
    var month = 12 + 1 - ruleArr[0];
    var oldDay = ruleArr[2] - ruleArr[1] + 1;
    var thirtyMon = getConsecutiveMonths(ruleArr[3]);
    var twentyNineMon = getConsecutiveMonths(ruleArr[4]);
    var nowMonth = 0;
    var isThirty = ruleArr[2] == 30 ? true : false;
    var leapMonth = ruleArr[5] ? ruleArr[5] : undefined;
    ...
    var getIsThirty = function () {
      if (thirtyMon.find(function (e) { return e == month; })) {
        if (isLeapMonth) {
          if (thirtyMon.find(function (e) { return String(e) == String(month + '.'); })) {
            isThirty = true;
          } else {
            isThirty = !isThirty;
          }
        } else if (thirtyMon.find(function (e) { return String(e) == String(month); })) {
          isThirty = true;
        } else {
          isThirty = !isThirty;
        }
      } else if (ruleArr[4] && twentyNineMon.find(function (e) { return e == month; })) {
        if (isLeapMonth) {
          if (twentyNineMon.find(function (e) { return String(e) == String(month + '.'); })) {
            isThirty = false;
          } else {
            isThirty = !isThirty;
          }
        } else if (twentyNineMon.find(function (e) { return String(e) == String(month); })) {
          isThirty = false;
        } else {
          isThirty = !isThirty;
        }
